app.controller('FlagController', ['$rootScope','$scope', '$routeParams', '$location', 'HttpService', '$http', 'FlashService','$window', function( $rootScope,$scope,$routeParams,$location,HttpService,$http,FlashService,$window ){
    var vm = this;
            ...
            .then(function(response){
                if (response.status == '200') {
                    ...
                    if (response && response.data && response.data["_id"]) {
                        FlashService.Success("The post is flagged for review.");
                        $rootScope.is_flagged = true;
                    }
                    $rootScope.loading = false;
                }else{
